...
/**
 * Triggers a browser download of the processed JSONL file.
 * Uses a temporary <a> element — no page navigation.
 */
export async function downloadDataset(id: number, filename: string): Promise<void> {
  ...
  a.href     = url;
  a.download = `${filename.replace(/\s+/g, "_")}_processed.jsonl`;
