...
async def chat_completions(
    ...
    try:
        ...
        if model_record.status.upper() not in {"READY", "COMPLETED"}:
            raise HTTPException(
                status_code=400,
                detail=f"Model is not ready. Current status: {model_record.status}",
            )
...
async def list_models(
    ...
    return {
        ...
        "data": [
            {
                "id": f"model_{m.id}" if not m.is_base_model else m.display_name.lower().replace(" ", "-"),
                "object": "model",
